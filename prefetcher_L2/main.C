...
//	int accessTimeMem = 50; 
	...
	memQueue writeBuffer(8,&DCache,accessTimeL2,true,true,'a');
...
//		printf("%u: %u\n",curr_cycle,cpu_status);
		...
		if(cpu_status == READY) { // request is ready
			...
			assert(req.fromCPU==true); //assert if all the request are from CPU
			// check for L1 hit
	...
	//mine
			...
			if(isHit && is_pre){L1_pf_used++;} //for every D-hit check whether it is due to prefetch
	
			cpu.hitL1(isHit);
			req.HitL1 = isHit;

/****MODIFIED******/	// notify the prefetcher of what just happened with this memory op
			//pf.cpuRequest(req);

			if(isHit) {
				DCache.access(req.addr,req.load, req.fromCPU);//accessing to update the LRU policy
...
//mine insert the pref.request in pref_table
		...
		if(queueL2.frontReady(curr_cycle)) { // check to see if the front element in the queue is ready
			...
			assert(req.fromCPU==true); //assert if the request is from CPU
			if ( req.load==true && req.fromCPU==false){
					printf("LOL found pref from L2Q\n");
					}
			isHit = L2Cache.check(req.addr,req.load);
			cpu.loadHitL2(isHit);
			req.HitL2 = isHit; //update that L2 is hit
// notify the prefetcher of what just happened with this memory ops
		      	if(req.load && req.fromCPU == true) pf.cpuRequest(req); //L2 was a miss so feed it to the prefetcher to learn 
			...
			if(queueMem.add(req,curr_cycle)) {
				...
				if (is_pre && req.fromCPU==false){L2_req_from_pref_is_miss++;}
...
//mine				
		...
		if(queueMem.frontReady(curr_cycle)) {
			...
			mem_req_from_L2++;
			if (req.fromCPU==false){mem_req_from_pref++;}
			queueMem.remove();

			// update both L2 and D cache
			L2Cache.access(req.addr,req.load,req.fromCPU);
			if(req.load) 
				DCache.access(req.addr,req.load,req.fromCPU); // only update if this is a load
...
//	printf("L1_miss  prefetch hit %d\n",pref_hit);
	...
	printf("PREFETCH_ACCURACY_L1	 %f\n",float(L1_pf_used)/mem_req_from_pref);
	printf("PREFETCH_ACCURACY_L2	 %f\n",float(L2_pf_used)/mem_req_from_pref);
	return 0;
}
